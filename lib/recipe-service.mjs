...
import {defaultRecipes,rank,assess,validateInput,key} from './recipe-candidates.mjs';
import {createICook} from './icook.mjs';
export const pairingRules=[
 {matches:/湯|燉|咖哩|燴/,directions:['白飯']},
 {matches:/涼拌|切絲|沙拉/,directions:['涼麵']},
 {matches:/煎|炒|烤|蒸/,directions:['炒飯','麵食']},
];
export function directions(recipe){return [...new Set(pairingRules.filter(r=>r.matches.test(recipe.title)).flatMap(r=>r.directions))].slice(0,2);}
...
export function createRecipeService(provider=createICook()){
 return {async recommend(input){
  ...
  if(input.pairMeals&&!failed){
   for(const main of candidates.filter(r=>!/飯|麵|粥/.test(r.title)).slice(0,2)){
    const direction=directions(main)[0];if(!direction||requests>=5)continue;
    const evidence=await search(direction);if(failed)break;
    const staple=evidence.find(r=>r.id!==main.id&&(direction==='麵食'?/麵/.test(r.title):key(r.title).includes(key(direction)))&&!assess(r,input).rejected);
