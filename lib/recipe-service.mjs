...
export function pairingCandidate(main,evidence,direction){return {...evidence,id:'pair:'+main.id+':'+direction+':'+evidence.id,source:'pair',sourceUrl:null,title:main.title+' → '+direction,author:null,note:'搭配方向由外部組合食譜支持；以該組合食譜本身核對材料，不重複合併原主菜。',parts:[evidence],evidenceUrl:evidence.sourceUrl};}
 ...
 return {async recommend(input){
  ...
  if(input.pairMeals&&!failed){
   for(const main of candidates.filter(r=>!/飯|麵|粥/.test(r.title)&&directions(r).length).slice(0,2)){
    ...
    if(!staple)continue;const assessment=assess(staple,input);if(!assessment.rejected)pairings.push({...pairingCandidate(main,staple,direction),assessment});
   }
  }
