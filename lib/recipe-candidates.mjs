...
// Seasoning/garnish differences should not turn the same dish into a new family.
// Keep substantive ingredients, including free-form names outside our pantry catalog.
export function principalIngredients(r){
 return [...new Set(r.ingredients.filter(i=>!['water','salt','oil','soySauce','sugar'].includes(i.internalId))
  .map(i=>key(INGREDIENTS.find(v=>v.id===i.internalId)?.name||i.name))
  .filter(n=>n&&!/^(?:蔥|青蔥|蔥花|蒜|大蒜|蒜末|薑|薑片|胡椒|黑胡椒|白胡椒|香菜|醋|米酒|香油|麻油)$/.test(n)))].sort();
}
export function family(r){
 const t=key(r.title).replace(/滑蛋/g,'炒蛋');
 const method=['湯麵','炒飯','涼麵','烘蛋','蒸蛋','蛋花湯','湯','煎','炒','燉','滷','烤','蒸','涼拌'].find(s=>t.includes(s))||t;
 return method+'|'+principalIngredients(r).join('|');
}
