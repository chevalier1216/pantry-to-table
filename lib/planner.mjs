...
export function parseInput(text){
 ...
 const noRestriction=/(?:無忌口|沒有忌口|忌口[:：\s]*(?:無|沒有)|不忌口)/g;
 text=text.replace(noRestriction,'');
 ...
 for(const item of INGREDIENTS){
  ...
  const segment=clean.slice(0,at).split(/[，,。；;、\n]/).pop()+clean.slice(at).split(/[，,。；;、\n]/)[0];
  const descriptive=()=>{draft.inventory[item.id]={description:segment.trim().slice(0,150),sufficient:!/(?:不夠|不足|不太夠)/.test(segment)&&/(?:足夠|充足|夠用|很多|充裕)/.test(segment)};};
  if(!found){descriptive();continue;}
  ...
  if(category!==item.unit || amount===null){descriptive();continue;}
 ...
 if(draft.equipment.includes('any'))draft.equipment=['any'];
 draft.exclusions=[...new Set(draft.exclusions)];
 // Food outside the current catalog is not silently treated as usable stock.
...
export function planMeals(input){
 ...
 for(const [id,amount] of Object.entries(inventory)){
  const descriptive=amount&&typeof amount==='object'&&typeof amount.description==='string'&&amount.description.trim().length>0&&amount.description.length<=150&&typeof amount.sufficient==='boolean';
  if(!ids.has(id)||(!descriptive&&(!Number.isFinite(amount)||amount<0||amount>100000||(id==='egg'&&!Number.isInteger(amount)))))issues.push('請確認食材數量或描述。');
 }
 ...
 const possible=RECIPES.filter(r=>(equipment.includes('any')||r.equipment.every(t=>equipment.includes(t)||(t==='pan'&&equipment.includes('castIron'))))&&Object.keys(r.ingredients).every(id=>{const i=INGREDIENTS.find(i=>i.id===id);return !exclusions.includes(id)&&!i.tags.some(t=>exclusions.includes(t));}));
 const combinations=possible.map(r=>[r]);
 const plans=[];
 for(const recipes of combinations){
  ...
  const ingredients=Object.entries(totals).map(([id,amount])=>{
   const stock=inventory[id];const descriptive=stock&&typeof stock==='object';
   return {...INGREDIENTS.find(i=>i.id===id),amount,available:descriptive?null:stock||0,stockDescription:descriptive?stock.description:null,needsQuantityCheck:!!descriptive&&!stock.sufficient};
  });
  const quantityChecks=ingredients.filter(i=>i.needsQuantityCheck);
  const missing=ingredients.filter(i=>i.available!==null&&i.amount>i.available).map(i=>({...i,shortage:Math.round((i.amount-i.available)*100)/100}));
  if(mode==='strict'&&missing.length)continue;
  // Require at least one non-staple food on hand in shopping mode.
  if(!ingredients.some(i=>!['water','salt','oil','soySauce'].includes(i.id)&&(i.available===null||i.available>0)))continue;
  ...
  plans.push({id:recipes.map(r=>r.id).join('+'),title:recipes.map(r=>r.name).join(' ＋ '),recipes:scaled,ingredients,missing,quantityChecks,timeline,totalMinutes:cursor,cookingMinutes:cursor-shoppingMinutes,shoppingMinutes,people,score,hasStaple:recipes.some(r=>r.kind==='staple')});
 }
 plans.sort((a,b)=>a.score-b.score);
 return {plans:plans.slice(0,15),issues:plans.length?[]:['目前食譜庫沒有符合所有條件的餐點。請檢查食材數量、油鹽水、爐火與鍋具，或調整時間／補買模式。']};
}
