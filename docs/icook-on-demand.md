...
- canonical id 去重，依缺料／待核對項排序，再以料理類型分散；同一類型最多兩筆，番茄炒蛋變體不能霸佔清單。
- 套餐勾選後，僅前兩個適合的單菜各取一個主食方向查證，共用五次 search 上限，不做乘積窮舉。規則可擴充；目前以湯／燉／咖哩搭白飯、涼拌方向搭涼麵、煎炒烤蒸搭炒飯或麵食為起點。只有實際搜尋到符合方向且未違反已知條件的食譜才顯示搭配。搜尋命中是可行性線索，非完整套餐驗證。
...
- 點開後從站內 `/api/recipes/detail` 讀取單篇，顯示材料／步驟／作者／來源連結，React 以純文字呈現。沒有圖片下載或 HTML 注入。
...
最後程式版本：`npm run test:core` 21/21、`npm run typecheck` PASS、`npm test` 47/47（含 production Worker build 與站內 detail HTTP）。新增 focused cases 20 項，涵蓋 budget、dedupe、diversity、strict/shop、quantity、exclusion、equipment、cache、favorite reload/remove、failure fallback、pairing checkbox 與非窮舉。

Windows 首次完整測試因 Bash 不在 PATH，第二次因沙盒 mkdir 權限失敗；加入已安裝 Git Bash 路徑並在正常權限執行後通過。未修改建置腳本或刪除失敗測試。

限制：未執行瀏覽器點擊／視覺驗證；站內 detail 目前以解析測試及 production HTTP 測試驗證。真實網路僅檢查兩個代表頁，沒有跑多查詢 live 推薦或付費 API。尚未部署。
