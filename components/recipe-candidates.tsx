...
export function RecipeCandidates({input,onDefault}:{input:Conditions|null;onDefault:(id:string|null)=>void}){
 ...
 <Dialog open={open} onOpenChange={v=>{setOpen(v);if(!v)detailGeneration.current++}}><DialogContent className="recipe-dialog"><DialogHeader><DialogTitle>{combined?.title||'Recipe Detail'}</DialogTitle><DialogDescription>站內食譜詳細資料 · 依來源顯示，不推算未知份量。</DialogDescription></DialogHeader><p role="status">{detailMessage}</p>{assessment&&<div className="notice"><strong>{assessment.rejected?'不符合目前條件，請勿視為可完成推薦':assessment.checks.length?'仍有條件待核對':'符合目前已知條件'}</strong><p>{assessment.missing.length?'缺少：'+assessment.missing.join('、'):''}</p><p>{assessment.checks.join('；')}</p><ul>{assessment.needs.map((i,n)=><li key={n}>{i.name}：{i.amount===null?'指定人份用量未知':`${i.amount} ${i.unit||''}`}{i.shortage?`（缺 ${i.shortage} ${i.unit}）`:''}</li>)}</ul></div>}{!input&&details.length>0&&<p>尚未設定料理條件；收藏不代表目前可完成。</p>}{details.map(r=><article key={r.id}><h3>{r.title}</h3><p>{r.author&&`作者：${r.author} · `}原食譜 {r.servings??'未知'} 人份 · {r.totalMinutes??'未知'} 分鐘</p><p>設備：{r.equipment?.map((id:string)=>EQUIPMENT.find(e=>e.id===id)?.name||id).join('、')||'來源未提供完整設備資訊'}</p><h4>來源用量</h4><ul>{r.ingredients.map((i,n)=><li key={n}>{i.name}：{i.amount===null?('amountText' in i?String(i.amountText):'未提供精確用量'):`${i.amount} ${i.unit}`}</li>)}</ul><h4>作法</h4>{r.steps.length?<ol>{r.steps.map((s:string,n:number)=><li key={n}>{s}</li>)}</ol>:<p>來源未提供作法。</p>}<p>{r.note}</p>{r.sourceUrl&&<p><a href={r.sourceUrl} target="_blank" rel="noreferrer">來源：iCook 愛料理</a></p>}<Button onClick={()=>save(r)}>收藏這道食譜</Button></article>)}</DialogContent></Dialog>
 </section>;
}
